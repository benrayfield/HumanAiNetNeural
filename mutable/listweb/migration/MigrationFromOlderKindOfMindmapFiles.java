package mutable.listweb.migration;
...
public class MigrationFromOlderKindOfMindmapFiles{
	...
	*/
	public static void benRayfieldsImport2016(boolean secondRun){
		...
		for(String path : paths){
			...
			UnaryOperator<String> renamer = (String name)->{
				...
				if(ListwebUtil.charsLenOfEscapedName(name) > ListwebRoot.maxEscapedNameLen){
					//lg("Name: "+name);
					//... is escaped to 9 bytes because of filenames . and ..
					if(name.length() > ListwebRoot.maxEscapedNameLen-9){
						name = name.substring(0,ListwebRoot.maxEscapedNameLen-9);
					}
					//TODO optimize by looping from end and only doing substring once
					while(ListwebUtil.charsLenOfEscapedName(name) > ListwebRoot.maxEscapedNameLen-9
			...
			String fileContent = Text.bytesToString(mutable.util.Files.read(new File(path)));
			if(secondRun){ //second run will sort prilists using partialPrilists each as a Comparator<String>
				...
				ListwebRoot.addToEndOfPrilistIfNotExist(ListwebRoot.rootName, mindmapName);
			}
		...
		if(secondRun){
			...
			for(String name : comparators.keySet()){
				...
				ListwebRoot.sortPrilist(name, comparators.get(name));
			}
		}
		//if(1<2) throw new Todo();
		ListwebRoot.updateRootChars();
		for(String name : thingsToAddToPrilistPrilistAtEnd){
			ListwebRoot.addToEndOfPrilistIfNotExist(ListwebRoot.rootName, name);
		}
				...
				if(line.equals("")){
					...
					if(line.startsWith(".def[") && line.endsWith("]")){
						...
						for(String childName : prilist){
							...
							ListwebRoot.addToEndOfPrilistIfNotExist(name, childName);
						}
					}else throw new Err("Not a def or prilist line: "+line);
				}
				if(ListwebRoot.listweb.size()%1000==0) lg("Mindmap size: "+ListwebRoot.listweb.size());
			}
		}catch(Exception e){
			throw new Err(e);
		}
	...
	public static void addToDef(String parent, String sep, String def){
		String oldDef = ListwebRoot.def(parent);
		ListwebRoot.setDef(parent, oldDef.equals("") ? def : oldDef+sep+def);
