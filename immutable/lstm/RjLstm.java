...
public class RjLstm{
	...
	*/
	public RjLstm(int ins, int nodes, int outs, DoubleSupplier rand){
		this(
			...
			MatD.rjNodeVec(new double[nodes]), //double[] bi,
			MatD.rjNodeVec(new double[nodes]), //double[] bf, //FIXME no need to duplicate these zeros doubles
			MatD.rjNodeVec(new double[nodes]), //double[] bo,
			MatD.rjNodeVec(new double[nodes]), //double[] bc,
			MatD.rjOutVec(new double[outs]), //double[] outFeedforwardB,
			null //double[][] nmhp
		);
	}
	...
	*/
	public RjLstm learn_testDelayedUpdateOfWeights(
			int repeat, double learnRate, Unaflop outFunc, Loss lossFunc, RjLearnStep[][] sequences){
			...
			for(int r=0; r<repeat; r++){
				...
				for(int parallelIndex=0; parallelIndex<parallelSize; parallelIndex++){
					...
					double reportedLossTrain = Trainer.pass(
						Trainer.ignoreMatrix, Trainer.defaultStateResetter,
						learnRate, rjLstm, dataSeqs, applyTraining, lossFunc, lossFunc);
				}
				if(RjOptions.testDelayedUpdateOfWeights){
					Trainer.updateModelParams(rjLstm, learnRate); //testDelayedUpdateOfWeights skips it in pass
				}
			}
		}catch(Exception e){ throw new Error(e); }
	...
	public RjLstm learn(int repeat, double learnRate, Unaflop outFunc, Loss lossFunc, RjLearnStep... sequence){
	...
	*/
	public RjNodesState newRandomNodesStates(int parallelVecs, DoubleSupplier hiddenContextRands, DoubleSupplier cellContextRands){
		...
		for(int i=0; i<siz; i++){
			n.cellContext.a[i] = hiddenContextRands.getAsDouble();
			n.hiddenContext.a[i] = cellContextRands.getAsDouble();
		}
		return n;
	}
