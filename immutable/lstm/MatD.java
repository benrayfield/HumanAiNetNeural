...
public class MatD{
	...
	*/
	...
	public MatD(int rows, int cols){
		this(rows, new double[rows*cols], new double[rows*cols]);
	}
	
	public MatD(int rows, double[] a){
		this(rows, a, new double[a.length]);
	}
	
	public MatD(int rows, double[] a, double[] stepCache){
		this.rows = rows;
	...
	public MatD(int rows, int cols, DoubleSupplier fill){
		...
		for(int i=0; i<a.length; i++) a[i] = fill.getAsDouble();
		this.stepCache = new double[a.length];
	}
	...
	*/
	public static MatD rjInVec(double[] inVec){
		return new MatD(inVec.length, inVec, new double[inVec.length]); //FIXME is this backward?
	}
	
	/** see comment of rjInVec.
	FIXME: I'm unsure if rjInVec, rjOutVec, or rjNodeVec should have a nonzero stepcache.
	*/
	public static MatD rjOutVec(double[] outVec){
		return new MatD(outVec.length, outVec, new double[outVec.length]); //FIXME is this backward?
	}
	
	/** see comment of rjInVec. Example: bias of forget gate.
	FIXME: I'm unsure if rjInVec, rjOutVec, or rjNodeVec should have a nonzero stepcache.
	*/
	public static MatD rjNodeVec(double[] nodeVec){
		return new MatD(nodeVec.length, nodeVec, new double[nodeVec.length]); //FIXME is this backward?
	}
	
	public String toString(){
		double aAve = MathUtil.ave(a);
		double aDev = MathUtil.devGivenAve(aAve, a);
		double sAve = MathUtil.ave(stepCache);
		double sDev = MathUtil.devGivenAve(sAve, stepCache);
